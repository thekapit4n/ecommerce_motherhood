...
class ProductComments extends Module
{
	...
	protected function _postProcess()
	{
        ...
        elseif (Tools::isSubmit('submitBulkenableSelectionproductcomments'))
		{
			if(isset($_POST['productcommentsBox']))
			{
				$commentids =  $_POST['productcommentsBox'];
				foreach($commentids AS $id)
				{
					if(intval($id)<=0)continue;
					$comment = new ProductComment($id);
			        $comment->validate();
				}
			}
			else
	...
	public function viewAccess($disable = false){
		header("location: index.php?controller=AdminModules&token=".Tools::getAdminTokenLite('AdminModules')."&configure=productcomments&tab_module=front_office_features&module_name=productcomments");
	...
	public function renderModerateLists()
	{
		...
		if (Configuration::get('PRODUCT_COMMENTS_MODERATE'))
		{
			...
			if (version_compare(_PS_VERSION_, '1.6', '<'))
			{
				...
				$actions = array('approve', 'disapprove');
	...
	{
		$this->smarty->assign(array(
			'href' => $this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'&deleteproductcomments&module_name='.$this->name.'&id_product_comment='.$id,
			'onclick' => "if (confirm('Delete selected item?\\n\\nName $name')){return true;}else{event.stopPropagation(); event.preventDefault();};",
			'action' => $this->l('Disapprove'),
		));

		return $this->display(__FILE__, 'views/templates/admin/list_action_disapprove.tpl');
	}
