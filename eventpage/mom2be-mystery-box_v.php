...
<!-- 
   ...
   -->
<div class="row mb-4">
   <div class="col-lg-12 col-md-12" style="padding-left: 0px; padding-right: 0px; margin-bottom: 0;"><img src="https://via.placeholder.com/1170x481.png?text=bannerhere" width="100%" /></div>
</div>
<div class="row mb-2">
   <div class="col-md-12 px-2"><label for="newemail" class="form-label required">Email</label> <input type="email" class="form-control" id="newemail" name="newEmail" value="{{predefine-email}}" required="" /> <input type="hidden" class="form-control statuscodeemail" /> <input type="hidden" class="form-control statusedd" /> <small class="error-msg"></small></div>
</div>
<div class="row mb-2">
...
<div class="row mb-2">
   ...
   <div class="col-md-6 px-2"><label for="parentdob" class="form-label required">Your Date of Birth</label> <input type="date" class="form-control" id="parentdob" name="subscriber_question2" required="" style="min-width: 100%;" /> <small class="error-msg"></small></div>
</div>
...
<div class="row mb-2">
   <div class="col-md-6 px-2">
      ...
      <select id="currentmilkbrand" class="form-select" name="subscriber_question9" style="height: 37px;">
         <option value="">Please select</option>
         {{option-listmilk}}
      </select>
      <small class="error-msg"></small>
   </div>
   <div class="col-md-6 px-2 edd-div"><label for="pregnancy-month" class="form-label">Estimated Date of Delivery (EDD)</label> <input type="date" class="form-control" id="pregnancy-month" name="subscriber_question12" style="min-width: 100%;" /> <small class="error-msg"></small></div>
</div>
<div class="row mb-2">
...
<div class="row my-4">
   <div class="col-md-12 px-2">
      <div class="form-check"><input class="form-check-input noUniform" type="checkbox" value="yes" id="tnccheckbox" name="subscriber_question13" required="" /> <label class="form-check-label noUniform px-3" for="tnccheckbox">
		T&C: By submitting this form, I hereby agree to the <a href="ttps://docs.google.com/document/d/1vMWXCVo031Cq3aGbru3CGRzANorp7bfm/edit?usp=sharing&ouid=106820278788873808665&rtpof=true&sd=true" target="_blank">Terms & Conditions</a> and the processing of my personal data in accordance with the terms of the Privacy Policy. Enlinea Sdn Bhd (Motherhood.com.my) and its sponsors may collect my personal details and use for communication purposes that include but are not limited to club memberships, Motherhood.com.my products, and Motherhood.com.my brands only.
	  </label>
	  </div>
   </div>
</div>
...
<p>
   <script>// <![CDATA[
      var errorSelector   = '.error-msg';
      		...
      		var validateEDD = function(){
      			var edd = $('body').find('#pregnancy-month').val();
      			$('#pregnancy-month').closest('.edd-div').find(errorSelector).html("");
      			$('body').find('#pregnancy-month').removeClass('is-invalid');
      			if(edd != undefined && edd != '')
      			{
      				$.ajax({
      					url		 : '../modules/enlineamixmod/enlineamixmod-edd-newmom-ajax.php', 
      					data	 :{'edd': edd, 'eventid' : eventID},
      					dataType :'json',
      					method 	 : 'post',
      					success	 : function(result){
      						if(result.status == true)
      						{
      							$('#pregnancy-month').closest('.edd-div').find(errorSelector).html("Current pregnancy week: " + result.data.week + " week(s)");
      							$('body').find('.statusedd').val(true);
      							return true;
      						}
      						else
      						{
      							$('body').find('#pregnancy-month').addClass('is-invalid');
      							$('#pregnancy-month').closest('.edd-div').find(errorSelector).html("<font style='color:#e9322d'>" + icon + " invalid EDD - " + result.msg + "</font>");
      							$('body').find('.statusedd').val(false);
      							return false;
      						}
      					}
      				});
      			...
      			else
      			{
      				...
      				$('body').find('#pregnancy-month').addClass('is-invalid');
      				$('#pregnancy-month').closest('.edd-div').find(errorSelector).html("<font style='color:#e9322d'>" + icon + " EDD is empty. Please insert EDD </font>");
      				$('body').find('.statusedd').val(false);
      				return false;
      			}
      		}
              ...
              $(function(){
      			...
      			$('body').on('blur', '#pregnancy-month', function(){
            			validateEDD();
            		});
                  ...
                  $('body').on('click', '.btn-submit-display', function(){
            			...
            			var validate_edd		= $('body').find('.statusedd').val();
            			...
            			console.log("validateEDD " + validate_edd  +  typeof validate_edd);
      				...
      				if(validate_edd == "false" || validate_edd == '')
      				{
      					$('body').find('#pregnancy-month').addClass('is-invalid');
      					flagcheck = false;
      				}
